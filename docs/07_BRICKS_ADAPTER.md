...
The normalization lifecycle recorded in provenance is:
    ...
    -> document_assembled -> ir_validated -> serialized

The normalizer reports normalization_status as serialized. The public
LiveFrames.Adapters.Bricks.to_ir/2 boundary validates the structured source,
assembles the Design IR document, and validates deterministic serialization. It
does not write the committed artifact, compare regenerated bytes, know the
caller's output path, or claim repository drift verification.

The repository artifact lifecycle is separate and is owned by the generation,
drift-test, repository-gate, and CI workflow:

    generated -> drift_verified -> completed

The generated artifact and its byte comparison are evidence for that external
lifecycle. These later states are not embedded in the DesignDocument
provenance returned by the pure normalizer.
